...
fn initialize_runtime(state: &mut GuestRuntime) -> Result<(), GuestErrorDetail> {
    ...
    let result: Result<(), GuestErrorDetail> = state.context.with(|ctx| {
        ...
        ctx.eval::<(), _>(RUNTIME_SURFACE)
            .map_err(|error| describe_exception(&ctx, "runtime surface installation", error))?;
...
/// Standards-oriented APIs that the QuickJS guest does not provide but
/// browser-targeted application bundles can use during module evaluation.
/// This stays guest-local so preinitialization never needs a host call.
const RUNTIME_SURFACE: &str = include_str!("js/runtime_surface.js");
...
mod tests {
    ...
    use super::RUNTIME_SURFACE;

    fn eval(source: &str) -> rquickjs::Result<()> {
    ...
    #[test]
    fn installs_utf8_and_base64_web_apis() -> rquickjs::Result<()> {
        eval(
            r#"
            const encoder = new TextEncoder();
            if (encoder.encoding !== "utf-8") throw new Error("wrong encoder encoding");
            const encoded = encoder.encode("A€😀\uD800");
            if (!(encoded instanceof Uint8Array)) throw new Error("wrong encode type");
            if (Array.from(encoded).join(",") !== "65,226,130,172,240,159,152,128,239,191,189") {
              throw new Error("wrong UTF-8 bytes");
            }
            const destination = new Uint8Array(4);
            const into = encoder.encodeInto("😀x", destination);
            if (into.read !== 2 || into.written !== 4 || Array.from(destination).join(",") !== "240,159,152,128") {
              throw new Error("wrong encodeInto result");
            }

            const decoder = new TextDecoder("UTF8");
            if (decoder.encoding !== "utf-8" || decoder.fatal || decoder.ignoreBOM) {
              throw new Error("wrong decoder properties");
            }
            if (decoder.decode(encoded) !== "A€😀�") throw new Error("wrong UTF-8 decode");
            if (new TextDecoder().decode(new Uint8Array([0xe2, 0x82])) !== "�") {
              throw new Error("incomplete UTF-8 sequence was not replaced once");
            }
            if (new TextDecoder().decode(new Uint8Array([0xef, 0xbb, 0xbf, 0x41])) !== "A") {
              throw new Error("BOM was not removed");
            }
            if (new TextDecoder("utf-8", { ignoreBOM: true }).decode(new Uint8Array([0xef, 0xbb, 0xbf, 0x41])) !== "\uFEFFA") {
              throw new Error("BOM was unexpectedly removed");
            }

            if (btoa("Convex\xFF") !== "Q29udmV4/w==") throw new Error("wrong btoa");
            if (atob("Q29u\n d m V 4 / w = =") !== "Convex\xFF") throw new Error("wrong atob");
            let rejected = false;
            try { btoa("€"); } catch (error) { rejected = error.name === "InvalidCharacterError"; }
            if (!rejected) throw new Error("btoa accepted non-Latin-1 input");
            "#,
        )
    }

    #[test]
    fn decoder_handles_streaming_and_fatal_errors() -> rquickjs::Result<()> {
        eval(
            r#"
            const decoder = new TextDecoder();
            if (decoder.decode(new Uint8Array([0xf0, 0x9f]), { stream: true }) !== "") {
              throw new Error("incomplete stream produced output");
            }
            if (decoder.decode(new Uint8Array([0x98, 0x80]), { stream: false }) !== "😀") {
              throw new Error("streaming UTF-8 decode failed");
            }
            let rejected = false;
            try { new TextDecoder("utf-8", { fatal: true }).decode(new Uint8Array([0xff])); }
            catch (error) { rejected = error instanceof TypeError; }
            if (!rejected) throw new Error("fatal decoder accepted invalid input");
            "#,
        )
    }
}
