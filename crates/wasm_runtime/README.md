...
The prototype is a workspace member but is intentionally omitted from the
workspace root's default package set because its Wasmtime/Wizer build is
compiler-heavy. Select it explicitly when comparing runtimes; the regular
backend build and test commands do not compile it:
