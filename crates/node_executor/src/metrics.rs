...
register_convex_histogram!(
    ...
    "Legacy Node package-acquisition duration, including cache lookup, materialization, and \
     cache-bound cleanup; this is not network-only download time"
);
...
register_convex_histogram!(
    LOCAL_NODE_REQUEST_SECONDS,
    "Time from local Node HTTP dispatch through terminal local response processing, including \
     exact-generation direct-child retirement synchronously awaited on the request path",
    &["pool_name", "request_kind", "outcome"]
);
pub fn log_local_node_request_time(
    pool_name: &str,
    request_kind: &'static str,
    elapsed: Duration,
    outcome: &'static str,
...
register_convex_gauge_evictable!(
    LOCAL_NODE_EXECUTOR_RESIDENT_SOURCE_PACKAGE_OWNERS_INFO,
    "Generation-resident source-package owners in the current local Node executor generation",
    &["pool_name"]
);
register_convex_gauge_evictable!(
...
pub fn set_local_node_package_state(
    ...
    match resident_source_owners {
        Some(resident_source_owners) => log_gauge_with_labels(
            &LOCAL_NODE_EXECUTOR_RESIDENT_SOURCE_PACKAGE_OWNERS_INFO,
            resident_source_owners as f64,
            labels(),
        ),
        None => {
            let _ = LOCAL_NODE_EXECUTOR_RESIDENT_SOURCE_PACKAGE_OWNERS_INFO
...
pub fn clear_local_node_package_state(pool_name: &str) {
    for gauge in [
        &*LOCAL_NODE_EXECUTOR_IMPORTED_SOURCE_PACKAGES_INFO,
        &*LOCAL_NODE_EXECUTOR_RETAINED_SOURCE_PACKAGES_INFO,
        &*LOCAL_NODE_EXECUTOR_RETAINED_SOURCE_PACKAGE_BYTES,
...
register_convex_histogram!(
    ...
    "Source-package lease acquisition, including materialization waits and cache-bound cleanup",
    &["pool_name", "request_kind", "cache_result", "outcome"]
);
pub fn log_local_node_package_acquire(
    pool_name: &str,
    request_kind: &'static str,
    ...
    log_distribution_with_labels(
        ...
        vec![
            StaticMetricLabel::new("pool_name", pool_name.to_owned()),
            StaticMetricLabel::new("request_kind", request_kind),
            StaticMetricLabel::new("cache_result", cache_result),
            StaticMetricLabel::new("outcome", outcome),
        ],
    );
}
...
register_convex_histogram!(
    LOCAL_NODE_PACKAGE_VALIDATION_SECONDS,
    "Validation of staged or prebuilt local Node package trees before registration or publication",
    &["pool_name", "package_kind", "phase", "outcome"]
);
pub fn log_local_node_package_validation(
    pool_name: &str,
    package_kind: &'static str,
    phase: &'static str,
    elapsed: Duration,
...
register_convex_histogram!(
    LOCAL_NODE_PACKAGE_CLEANUP_SECONDS,
    "Awaited retirement of unowned local Node package trees",
    &["pool_name", "package_kind", "outcome"]
);
pub fn log_local_node_package_cleanup(
    pool_name: &str,
    package_kind: &'static str,
    elapsed: Duration,
    outcome: &'static str,
...
register_convex_counter!(
    ...
    "Package-stage observations overwritten before a successful child health snapshot",
    &["pool_name"]
);
pub fn log_local_node_package_stage_observations_dropped(pool_name: &str, count: u64) {
    if count == 0 {
        return;
    }
    log_counter_with_labels(
...
register_convex_histogram!(
    LOCAL_NODE_SYSTEM_ADMISSION_WAIT_SECONDS,
    "Time from a local Node system-operation admission request to reservation or terminal wait \
     outcome",
    &["request_kind", "outcome"]
);
pub fn log_local_node_system_admission_wait(
    request_kind: &'static str,
    elapsed: Duration,
    outcome: &'static str,
) {
...
register_convex_counter!(
    LOCAL_NODE_SYSTEM_RESULT_DELIVERY_TOTAL,
    "Original-caller result delivery outcomes for dispatched local Node system operations",
    &["request_kind", "outcome"]
);

#[derive(Clone, Copy)]
pub(crate) enum SystemResultDeliveryOutcome {
    Delivered,
    CallerAbandoned,
    OperationTaskLost,
}

impl SystemResultDeliveryOutcome {
    fn as_str(self) -> &'static str {
        match self {
            Self::Delivered => "delivered",
            Self::CallerAbandoned => "caller_abandoned",
            Self::OperationTaskLost => "operation_task_lost",
        }
    }
}

pub(crate) fn log_local_node_system_result_delivery(
    request_kind: &'static str,
    outcome: SystemResultDeliveryOutcome,
) {
    log_counter_with_labels(
        &LOCAL_NODE_SYSTEM_RESULT_DELIVERY_TOTAL,
        1,
...
register_convex_gauge!(
    LOCAL_NODE_SYSTEM_ACTIVE_INFO,
    "Actual operation ownership of the serialized local Node system slot",
    &["request_kind"]
);
pub fn set_local_node_system_active(active_kind: Option<&'static str>) {
    for request_kind in ["analyze", "build_deps"] {
        log_gauge_with_labels(
            &LOCAL_NODE_SYSTEM_ACTIVE_INFO,
            f64::from(active_kind == Some(request_kind)),
            vec![StaticMetricLabel::new("request_kind", request_kind)],
        );
    }
}

register_convex_gauge!(
    LOCAL_NODE_SYSTEM_WAITING_INFO,
    "Callers waiting for the serialized local Node system slot",
    &["request_kind"]
);
pub fn set_local_node_system_waiting(request_kind: &'static str, waiting: usize) {
    log_gauge_with_labels(
        &LOCAL_NODE_SYSTEM_WAITING_INFO,
        waiting as f64,
        vec![StaticMetricLabel::new("request_kind", request_kind)],
...
register_convex_histogram!(
    LOCAL_NODE_PREPARE_SECONDS,
    "Rust-observed local Node package preparation through validated acknowledgment",
    &["pool_name", "purpose", "outcome"]
);
pub fn log_local_node_prepare(
    pool_name: &str,
    purpose: &'static str,
    elapsed: Duration,
    outcome: &'static str,
    ...
    log_distribution_with_labels(
        ...
        vec![
            StaticMetricLabel::new("pool_name", pool_name.to_owned()),
            StaticMetricLabel::new("purpose", purpose),
            StaticMetricLabel::new("outcome", outcome),
        ],
    );
}
...
register_convex_counter!(
    LOCAL_NODE_RESIDENT_READINESS_TOTAL,
    "Resident local Node package preparation outcomes",
    &["pool_name", "result"]
);
pub fn log_local_node_resident_readiness(pool_name: &str, result: &'static str) {
    log_counter_with_labels(
        &LOCAL_NODE_RESIDENT_READINESS_TOTAL,
        1,
        vec![
            StaticMetricLabel::new("pool_name", pool_name.to_owned()),
            StaticMetricLabel::new("result", result),
        ],
    );
}
...
mod tests {
    ...
    #[test]
    fn missing_resident_owner_telemetry_removes_the_gauge() {
        let pool_name = "missing_resident_owner_telemetry_test";
        set_local_node_package_state(pool_name, 0, 0, 0, 0, Some(1), 0, 0, 0);
        set_local_node_package_state(pool_name, 0, 0, 0, 0, None, 0, 0, 0);
        assert!(LOCAL_NODE_EXECUTOR_RESIDENT_SOURCE_PACKAGE_OWNERS_INFO
            .remove_label_values(&[pool_name])
            .is_err());
    }
}
