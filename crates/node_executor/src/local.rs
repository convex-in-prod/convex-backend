...
use crate::{
    executor::{
        ...
        InvokeResponse,
        NodeExecutor,
        NodeExecutorResponseProtocolError,
...
const MAX_PACKAGE_STAGE_OBSERVATIONS: usize = 256;
...
pub(crate) enum LocalNodeExecutorRole {
    Application,
    System,
}

impl LocalNodeExecutorRole {
...
struct ExecutorPoolActivity {
    pool_name: Arc<str>,
    // Keep the count update and gauge publication together. Separate atomic
    // updates can publish an older value after a later concurrent request has
    // already refreshed the same pool label.
    waiting_requests: StdMutex<usize>,
    active_requests: StdMutex<usize>,
}
    ...
    fn drop(&mut self) {
        ...
        assert!(
            state
                ...
                .as_ref()
                .is_some_and(|occupant| occupant.id == self.id),
            "Local Node surge permit lost coordinator ownership before confirmed direct-child \
             reaping"
        );
        drop(state);
...
enum PreparationResponse {
    Success {
        residency: PreparationResidency,
...
impl PreparationMode {
    ...
    fn expected_residency(self) -> PreparationResidency {
        match self {
            Self::Warm => PreparationResidency::Released,
            Self::Resident => PreparationResidency::Retained,
        }
    }
}
...
impl Drop for HotTransitionCleanupAttemptGuard {
    fn drop(&mut self) {
        if self.armed {
            // Cancellation and an unwinding panic must publish a terminal
            // attempt and make any concurrent retry request runnable again.
...
struct ManagedChild {
    ...
    cold_startup_owner: Option<Arc<ColdStartupOwner>>,
}

#[cfg(all(test, unix))]
pub(crate) struct TestLocalNodeChildLock {
    _guard: OwnedMutexGuard<ManagedChild>,
}

struct ColdStartupOwner {
    pool_name: Arc<str>,
    generation: u64,
    child_attached: AtomicBool,
    startup_guard: StdMutex<Option<OwnedMutexGuard<()>>>,
}
...
struct PreparationTimingObservation<'a> {
    pool_name: &'a str,
    purpose: &'static str,
    started_at: Instant,
    outcome: &'static str,
...
impl Drop for PreparationTimingObservation<'_> {
    fn drop(&mut self) {
        crate::metrics::log_local_node_prepare(
            self.pool_name,
            self.purpose,
            self.started_at.elapsed(),
            self.outcome,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum ResidentGenerationAccess {
    AllowColdStart,
    /// Routed execution has already selected and fenced its target. It may
    /// cold-start an empty slot, but a different resident target indicates a
    /// concurrent cutover rather than permission to retarget that cutover.
    AllowColdStartWithoutRetarget,
    // A stale router snapshot may continue only on the generation it already
    // observed. It must not recreate that snapshot after proactive retirement.
    RequireExisting,
}

impl ResidentGenerationAccess {
    fn allows_empty_cold_start(self) -> bool {
        !matches!(self, Self::RequireExisting)
    }

    fn allows_cold_start_after_generation_loss(self) -> bool {
        matches!(self, Self::AllowColdStart)
    }

    fn allows_fingerprint_replacement(self) -> bool {
        matches!(self, Self::AllowColdStart)
    }
}
...
struct InnerLocalNodeExecutor {
    ...
    // A fingerprint selects the package identity, but it is not residency
    // authority until `/prepare` confirms that the child retained the lease.
    resident_package_ready: AtomicBool,
    activity: Arc<ExecutorPoolActivity>,
    pid: u32,
    started_at: Instant,
    runtime_stats_supported: bool,
    resident_source_owners_supported: bool,
    active_requests: AtomicUsize,
    retirement_requested: AtomicBool,
    retirement_reason: StdMutex<Option<GenerationRetirementReason>>,
    idle: Notify,
    ...
    retired: AtomicBool,
    // Direct-child reaping and removal from the generation slot are separate
    // steps. Keep the confirmed reap authoritative if the retirement task is
    // canceled between them, so a retry does not turn successful cleanup into
    // an "already reaped" failure.
...
impl Drop for RetirementTaskGuard {
    fn drop(&mut self) {
        ...
        // aborted or unwinds, so shutdown and replacement waiters cannot hang.
...
struct NodePackageCacheStats {
    ...
    // Resident ownership is a later aggregate extension. Missing means not
    // emitted; explicit null remains malformed rather than becoming zero.
    #[serde(default, deserialize_with = "deserialize_present_runtime_stats")]
    resident_source_owners: Option<u64>,
    ...
    #[serde(default)]
    package_stage_observation_sequence: u64,
    #[serde(default)]
    package_stage_observations_dropped: u64,
    #[serde(default)]
    package_stage_observations: Vec<NodePackageStageObservation>,
}

#[derive(Deserialize)]
#[serde(tag = "stage", rename_all = "snake_case")]
enum NodePackageStageObservation {
    Acquire {
        sequence: u64,
        #[serde(rename = "requestKind")]
        request_kind: NodePackageRequestKind,
        #[serde(rename = "cacheResult")]
        cache_result: NodePackageCacheResult,
        outcome: NodePackageStageOutcome,
    ...
    Validation {
        ...
        package_kind: NodePackageKind,
        phase: NodePackageValidationPhase,
        outcome: NodePackageStageOutcome,
        #[serde(rename = "durationMs")]
        duration_ms: f64,
    },
...
#[serde(rename_all = "snake_case")]
enum NodePackageRequestKind {
    Execute,
    Analyze,
    Prepare,
}

impl NodePackageRequestKind {
    fn as_str(self) -> &'static str {
        match self {
...
#[derive(Clone, Copy, Deserialize)]
#[serde(rename_all = "snake_case")]
enum NodePackageCacheResult {
    CurrentHit,
    JoinedMaterialization,
    NewMaterialization,
    RetirementWait,
}

impl NodePackageCacheResult {
    fn as_str(self) -> &'static str {
        match self {
            Self::CurrentHit => "current_hit",
            Self::JoinedMaterialization => "joined_materialization",
            Self::NewMaterialization => "new_materialization",
            Self::RetirementWait => "retirement_wait",
        }
    }
}
...
#[derive(Clone, Copy, Deserialize)]
#[serde(rename_all = "snake_case")]
enum NodePackageKind {
    Source,
    External,
}

impl NodePackageKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::Source => "source",
...
#[derive(Clone, Copy, Deserialize)]
#[serde(rename_all = "snake_case")]
enum NodePackageValidationPhase {
    Staged,
    Prebuilt,
}

impl NodePackageValidationPhase {
    fn as_str(self) -> &'static str {
        match self {
            Self::Staged => "staged",
...
#[derive(Clone, Copy, Deserialize)]
#[serde(rename_all = "snake_case")]
enum NodePackageStageOutcome {
    Success,
    Failure,
}

impl NodePackageStageOutcome {
    fn as_str(self) -> &'static str {
        match self {
...
impl NodePackageStageObservation {
    fn sequence(&self) -> u64 {
        match self {
            Self::Acquire { sequence, .. }
            | Self::Validation { sequence, .. }
            | Self::Cleanup { sequence, .. } => *sequence,
        }
    }

    fn duration_ms(&self) -> f64 {
        match self {
            Self::Acquire { duration_ms, .. }
            | Self::Validation { duration_ms, .. }
            | Self::Cleanup { duration_ms, .. } => *duration_ms,
        }
    }
}
...
impl GenerationRetirementReason {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ResponseStreamError => "response_stream_error",
...
impl GenerationRetirementDiagnostics {
    ...
    fn system_operation_task_lost(request_kind: &'static str) -> Self {
        Self {
            reason: GenerationRetirementReason::SystemOperationTaskLost,
            request_kind,
...
impl ManagedChild {
    fn new(
        ...
        if let Some(owner) = &cold_startup_owner {
            owner.attach_child();
        }
        Self {
            generation,
    ...
    fn mark_reaped(&mut self) {
        ...
        if let Some(owner) = &self.cold_startup_owner {
            owner.finish();
        }
    }
    ...
    fn spawn_drop_cleanup(&mut self) {
        ...
        let cold_startup_owner = self.cold_startup_owner.take();
        ...
        runtime.spawn(async move {
            ...
            {
                // A published child has no cold-start capacity owner, so its
                // final fallback is kill-on-drop and Tokio's orphan reaper.
                // An unpublished cold child must retain ownership below.
                tracing::error!(
                    pool_name = %pool_name,
                    generation,
                    error_kind = ?error.kind(),
                ...
                if cold_startup_owner.is_some() {
                    // The next cold start must remain fenced when neither kill
                    // nor reaping was confirmed. Retain the exact child, files,
                    // and startup owner until runtime shutdown.
            ...
            match child.wait().await {
                ...
                Err(error) => {
                    ...
                    if cold_startup_owner.is_some() {
                        // A wait error is not evidence that the direct child
                        // exited. Preserve ownership and keep cold replacement
                        // capacity closed instead of relying on kill-on-drop.
                        std::future::pending::<()>().await;
                    }
                },
            }
...
impl ColdStartupOwner {
    fn new(pool_name: Arc<str>, generation: u64, startup_guard: OwnedMutexGuard<()>) -> Self {
        Self {
            pool_name,
            generation,
            child_attached: AtomicBool::new(false),
            startup_guard: StdMutex::new(Some(startup_guard)),
        }
    }

    fn attach_child(&self) {
        assert!(!self.child_attached.swap(true, Ordering::AcqRel));
    }
    ...
    fn finish(&self) {
        drop(
            self.startup_guard
                .lock()
                .expect("Local Node cold-start owner lock poisoned")
                .take(),
        );
    }
}

impl Drop for ColdStartupOwner {
    fn drop(&mut self) {
        if !self.child_attached.load(Ordering::Acquire) {
            return;
        }
        ...
        if let Some(startup_guard) = startup_guard {
            // Runtime teardown can cancel the detached reaper. Keep the lock
            // permanently closed in this executor rather than treating task
            // loss as confirmed direct-child cleanup.
            std::mem::forget(startup_guard);
            tracing::error!(
                pool_name = %self.pool_name,
                generation = self.generation,
                "Local Node cold-start ownership ended without confirmed direct-child reaping"
            );
        }
    }
}
...
impl NodeExecutorHealth {
    ...
    fn resident_source_owners_supported(&self) -> bool {
        self.package_cache
            .as_ref()
            .is_some_and(|package| package.resident_source_owners.is_some())
    }

    fn is_valid_generation_observation(
        &self,
        previous_package: &NodePackageCacheStats,
        previous_stack: &NodeStackTraceStats,
        runtime_stats_supported: bool,
        ...
        self.status == "ok"
            && self.valid_runtime_stats_support(previous_package, previous_stack)
                == Some(runtime_stats_supported)
            && self.resident_source_owners_supported() == resident_source_owners_supported
    }
    ...
    fn runtime_counters_are_monotonic(
        ...
        package.imported_source_packages >= previous_package.imported_source_packages
            ...
            && package.package_stage_observations_are_valid(previous_package)
            ...
            && Duration::try_from_secs_f64(stack.duration_ms / 1000.0).is_ok()
    }
}

impl NodePackageCacheStats {
    fn package_stage_observations_are_valid(&self, previous: &Self) -> bool {
        let expected_observation_count = self
            .package_stage_observation_sequence
            .min(MAX_PACKAGE_STAGE_OBSERVATIONS as u64);
        let Some(expected_dropped_count) = self
            .package_stage_observation_sequence
            .checked_sub(expected_observation_count)
        else {
            return false;
        };
        if self.package_stage_observation_sequence < previous.package_stage_observation_sequence
            || self.package_stage_observations_dropped < previous.package_stage_observations_dropped
            || self.package_stage_observations.len() != expected_observation_count as usize
            || self.package_stage_observations_dropped != expected_dropped_count
        {
            return false;
        }
        let Some(mut expected_sequence) = self.package_stage_observations_dropped.checked_add(1)
        else {
            return false;
        };
        for observation in &self.package_stage_observations {
            if observation.sequence() != expected_sequence
                || !observation.duration_ms().is_finite()
                || observation.duration_ms() < 0.0
                || Duration::try_from_secs_f64(observation.duration_ms() / 1000.0).is_err()
            {
                return false;
            }
        ...
        expected_sequence.saturating_sub(1) == self.package_stage_observation_sequence
    }
}
...
impl WaitingRequestGuard {
    fn new(activity: Arc<ExecutorPoolActivity>) -> Self {
        let mut waiting = activity
            .waiting_requests
            .lock()
            .expect("Local Node waiting-request count lock poisoned");
        *waiting = waiting
            .checked_add(1)
            .expect("Local Node waiting-request count overflow");
    ...
    fn finish(mut self) {
        ...
        *waiting = waiting
            .checked_sub(1)
            .expect("Local Node waiting-request count underflow");
        crate::metrics::set_local_node_waiting_requests(&self.activity.pool_name, *waiting);
        drop(waiting);
        self.waiting = false;
    }
}
...
impl ActiveRequestGuard {
    fn new(inner: Arc<InnerLocalNodeExecutor>, metadata: RequestDiagnosticMetadata) -> Self {
        let request_kind = metadata.request_kind;
        let mut guard = Self {
            inner,
            diagnostic_id: None,
            request_kind,
        ...
        let mut pool_active = guard
            .inner
            .activity
            .active_requests
            .lock()
            .expect("Local Node active-request count lock poisoned");
        *pool_active = pool_active
            .checked_add(1)
            .expect("Local Node active-request count overflow");
        crate::metrics::log_local_node_request_start(&guard.inner.pool_name);
        crate::metrics::set_local_node_active_requests(&guard.inner.pool_name, *pool_active);
        drop(pool_active);
        guard
    }
    ...
    fn start_request_timer(&mut self) {
        assert!(
            self.request_started_at.replace(Instant::now()).is_none(),
            "Local Node request timer was started twice"
        );
        // A dropped future after dispatch is caller/task abandonment, not an
        // internal result. Any detached retirement owner continues separately.
        self.outcome = "canceled_after_dispatch";
    }
}
...
impl Drop for ActiveRequestGuard {
    fn drop(&mut self) {
        if std::thread::panicking()
            && matches!(
                self.outcome,
                "canceled_before_start" | "canceled_after_dispatch"
            )
        {
            // Tokio drops the request future while unwinding a task panic;
            // neither pre-dispatch nor dispatched panic is caller cancellation.
        ...
        *pool_active = pool_active
            .checked_sub(1)
            .expect("Local Node active-request count underflow");
        crate::metrics::set_local_node_active_requests(&self.inner.pool_name, *pool_active);
        drop(pool_active);
        ...
        if let Some(started_at) = self.request_started_at {
            crate::metrics::log_local_node_request_time(
...
impl LocalNodeExecutorConfig {
    ...
    pub(crate) fn with_pool_name(mut self, pool_name: Arc<str>) -> anyhow::Result<Self> {
        ...
        self.memory_pressure_min_rss_bytes = u64::try_from(
            local_node_executor_pool_memory_pressure_min_rss_bytes(&pool_name),
        )
    ...
    pub(crate) fn into_system_role(mut self) -> anyhow::Result<Self> {
        ...
        self.max_rss_bytes = u64::try_from(*LOCAL_NODE_EXECUTOR_MAX_RSS_BYTES)
            .context("Local Node executor RSS threshold does not fit u64")?;
        self.memory_pressure_min_rss_bytes =
            u64::try_from(*LOCAL_NODE_EXECUTOR_MEMORY_PRESSURE_MIN_RSS_BYTES)
                .context("Local Node executor cgroup-pressure RSS threshold does not fit u64")?;
        self.validate()?;
        // System generations recycle cold and never acquire this coordinator.
        // Retain it so constructing the role cannot reset application-global
        // surge gauges through a second coordinator's initialization.
        Ok(self)
    }
...
impl InnerLocalNodeExecutor {
    ...
    async fn new(
        ...
        for _ in 0..MAX_HEALTH_CHECK_ATTEMPTS {
            ...
            let runtime_stats_support = health
                .as_ref()
                .filter(|health| health.status == "ok")
                .and_then(|health| {
                    health
                        .valid_runtime_stats_support(&empty_package_stats, &empty_stack_stats)
                        .map(|runtime_stats_supported| {
                            (
                                runtime_stats_supported,
                                health.resident_source_owners_supported(),
                            )
                        })
                });
            crate::metrics::log_local_node_health_check(
                ...
                "startup",
                runtime_stats_support.is_some(),
            );
            if let Some((runtime_stats_supported, resident_source_owners_supported)) =
                runtime_stats_support
            {
                return Ok(Self {
                    generation,
                    pool_name: config.pool_name.clone(),
                    resident_fingerprint,
                    resident_package_ready: AtomicBool::new(false),
                    activity,
                    pid,
                    started_at: Instant::now(),
                    runtime_stats_supported,
                    resident_source_owners_supported,
                    active_requests: AtomicUsize::new(0),
                    retirement_requested: AtomicBool::new(false),
                    retirement_reason: StdMutex::new(None),
                    idle: Notify::new(),
    ...
    async fn prepare_package_request(
        &self,
        ...
        {
            PreparationResponse::Success { residency, result } => {
                anyhow::ensure!(
                    residency == mode.expected_residency(),
                    "Local Node executor preparation response did not confirm requested residency"
                );
                Ok(result)
            },
    ...
    async fn prepare_package(
        ...
        let mut observation = PreparationTimingObservation {
            pool_name: &self.pool_name,
            purpose,
            started_at: Instant::now(),
            outcome: "canceled",
        ...
        let result = self
            .prepare_package_request(descriptor, mode, timeout)
            .await;
        observation.outcome = if result.is_ok() { "success" } else { "failure" };
        match result {
            Ok(preparation_result) => {
                if matches!(mode, PreparationMode::Resident) {
                    self.resident_package_ready.store(true, Ordering::Release);
                    crate::metrics::log_local_node_resident_readiness(
                        &self.pool_name,
                        preparation_result.as_str(),
                    );
                }
                Ok(preparation_result)
            },
...
impl LocalNodeExecutor {
    ...
    pub(crate) async fn finish_system_operation_task_loss(
        &self,
        request_kind: &'static str,
    ) -> anyhow::Result<()> {
        anyhow::ensure!(
            matches!(self.config.role, LocalNodeExecutorRole::System),
            "Only the Local Node system executor can clean up a lost system-operation task"
        );
        // A canceled cold start can own an unpublished direct child. Its
        // startup owner releases this lock only after that child is reaped.
        let _startup_guard = self.startup_lock.lock().await;
        // The router still holds exclusive system admission, and this role
        // never starts a hot replacement. Its current or retiring identity is
        // therefore the lost task's exact generation, if one still exists.
        let diagnostics = GenerationRetirementDiagnostics::system_operation_task_lost(request_kind);
        loop {
            let (expected, already_retiring) = {
                let state = self.state.lock().await;
                match (&state.inner, &state.retiring) {
            ...
            if already_retiring {
                let reason = expected
                    .retirement_reason
                    .lock()
                    .expect("Local Node retirement reason lock poisoned")
                    .expect("Retiring Local Node generation has no retirement reason");
                return Self::finish_retiring_inner_state(&self.state, &expected, reason).await;
            }
            match Self::retire_inner_state(&self.state, &expected, diagnostics).await {
                Ok(true) => return Ok(()),
                Ok(false) => continue,
                Err(error) => {
                    let exact_retirement_failed = self
                        .state
                        .lock()
                        .await
                        .retiring
                        .as_ref()
                        .is_some_and(|retiring| Arc::ptr_eq(retiring, &expected));
                    if !exact_retirement_failed {
                        return Err(error);
                    }
                    // Task loss can interrupt the first retirement owner too.
                    // Retry only this exact identity; repeated cleanup failure
                    // remains an error so the router keeps admission fenced.
                    return Self::finish_retiring_inner_state(
                        &self.state,
                        &expected,
                        diagnostics.reason,
                    )
    ...
    pub(crate) async fn finish_system_operation_terminal_observation(&self) -> anyhow::Result<()> {
        anyhow::ensure!(
            matches!(self.config.role, LocalNodeExecutorRole::System),
            "Only the Local Node system executor has a system-operation terminal boundary"
        );
        // An unpublished cold child keeps this lock through detached cleanup.
        // A normal operation cannot release router admission before that exact
        // owner confirms direct-child reaping.
        let _startup_guard = self.startup_lock.lock().await;
        loop {
            let (current, retiring) = {
                let state = self.state.lock().await;
                (state.inner.clone(), state.retiring.clone())
            };
            if let Some(retiring) = retiring {
                let reason = retiring
                    ...
                    .expect("Local Node retirement reason lock poisoned")
                    .expect("Retiring Local Node generation has no retirement reason");
                return Self::finish_retiring_inner_state(&self.state, &retiring, reason).await;
            }
            let Some(current) = current else {
                // The request reached a terminal local response observation. No
                // generation remains to keep system admission fenced.
                return Ok(());
            };
            if !current.retirement_requested.load(Ordering::Acquire) {
                // A healthy current generation remains reusable across
                // operations.
                return Ok(());
            }
            // Proactive system recycling can request a drain after the child
            // response arrives but before this observer acquires startup_lock.
            // Wait for that exact generation instead of releasing admission
            // while its direct child is still draining or being reaped.
            current.wait_until_retired().await?;
        }
    }
    ...
    pub(crate) async fn has_generation(&self) -> bool {
        self.state.lock().await.inner.is_some()
    }

    #[cfg(all(test, unix))]
    pub(crate) async fn install_test_generation(&self, client: Client) {
        assert!(matches!(self.config.role, LocalNodeExecutorRole::System));
        ...
        state.inner = Some(test_inner_with_owner(
    ...
    #[cfg(all(test, unix))]
    pub(crate) async fn lock_test_generation_child(&self) -> TestLocalNodeChildLock {
        let child = {
            let state = self.state.lock().await;
            state
                .inner
                ...
                .expect("Test Local Node executor has no generation")
                .server_handle
                .clone()
        };
        TestLocalNodeChildLock {
            _guard: child.lock_owned().await,
        }
    }

    #[cfg(all(test, unix))]
    pub(crate) async fn test_has_generation_owner(&self) -> bool {
        let state = self.state.lock().await;
        state.inner.is_some() || state.retiring.is_some()
    }

    #[cfg(all(test, unix))]
    pub(crate) async fn fail_next_test_generation_termination(&self) {
        self.fail_next_test_generation_terminations(1).await;
    }

    #[cfg(all(test, unix))]
    pub(crate) async fn fail_next_test_generation_terminations(&self, count: usize) {
        assert!(
            count > 0,
            "Test Local Node termination failure count is zero"
        );
        let state = self.state.lock().await;
        let inner = state
            .inner
            .as_ref()
            .expect("Test Local Node executor has no current generation");
        assert_eq!(
            inner
                .termination_failures_remaining
                .swap(count, Ordering::AcqRel),
            0,
            "Test Local Node executor already has an injected termination failure"
        );
    }
    ...
    #[cfg(all(test, unix))]
    pub(crate) async fn test_has_retiring_generation(&self) -> bool {
        self.state.lock().await.retiring.is_some()
    }
    ...
    pub(crate) async fn resident_fingerprint_matches(
        ...
        self.state.lock().await.inner.as_ref().is_some_and(|inner| {
            inner.resident_package_ready.load(Ordering::Acquire)
                && inner
                    .resident_fingerprint
                    .as_ref()
    ...
    pub(crate) async fn resident_fingerprint_matches_snapshot(
        &self,
        target: &ResidentGenerationFingerprint,
    ) -> bool {
        self.state.lock().await.inner.as_ref().is_some_and(|inner| {
            inner.resident_package_ready.load(Ordering::Acquire)
                && inner.resident_fingerprint.as_ref().is_some_and(|current| {
                    current.same_package_and_environment(target)
                        && current.topology_version <= target.topology_version
                })
        })
    }
    ...
    pub(crate) async fn wait_for_resident_package_and_environment_ready(
        ...
        // Keep the generation observed for this target until the wait ends.
        // Hard retirement removes it from `state.inner` before its cleanup
        // owner finishes, so looking only at state after the notification can
        // otherwise turn a terminal child loss into an indefinite wait.
        let mut observed_target = None;
        loop {
            let changed = self.transition_changed.notified();
            tokio::pin!(changed);
            changed.as_mut().enable();
            let (target_ready, target_unavailable, current_target, retiring_target) = {
                let state = self.state.lock().await;
                let current = state.inner.clone();
                let current_target = current.as_ref().filter(|current| {
                    current
                        .resident_fingerprint
                        .as_ref()
                        .is_some_and(|resident| resident.same_package_and_environment(target))
                });
                let retiring_target = state.retiring.as_ref().filter(|retiring| {
                    retiring
                        .resident_fingerprint
                        .as_ref()
                        .is_some_and(|resident| resident.same_package_and_environment(target))
                ...
                if observed_target.is_none() {
                    observed_target = current_target.cloned().or_else(|| retiring_target.cloned());
                }
                let target_ready = current_target.is_some_and(|current| {
                    current.resident_package_ready.load(Ordering::Acquire)
                        && !current.retirement_requested.load(Ordering::Acquire)
                        && !current.execution_unavailable.load(Ordering::Acquire)
                });
                // Prefer the current matching generation when a replacement
                // has already taken over; an older generation's loss must not
                // reject a ready equivalent replacement.
                let target_unavailable = current_target
                    .or(observed_target.as_ref())
                    .is_some_and(|current| current.execution_unavailable.load(Ordering::Acquire));
                (
                    target_ready,
                    target_unavailable,
                    current_target.cloned(),
                    retiring_target.cloned(),
                )
            };
            if target_ready {
                return Ok(());
            ...
            if target_unavailable {
                anyhow::bail!(
                    "Local Node executor target generation became unavailable before readiness"
                );
            }
            // A hard-loss notification is separate from transition publication:
            // the current generation may become unavailable after cutover has
            // completed, when no later transition notification is guaranteed.
            if let Some(current) = current_target
                .or(retiring_target)
                .or(observed_target.clone())
            {
                let unavailable = current.wait_until_execution_unavailable();
    ...
    pub(crate) async fn replace_for_deployment(
        &self,
        target_fingerprint: ResidentGenerationFingerprint,
        source_package: crate::executor::SourcePackage,
        reserved_permit: SurgePermit,
    ) -> anyhow::Result<DeploymentReplacementOutcome> {
        let target_fingerprint = Some(target_fingerprint);
        ...
        ) && expected.resident_package_ready.load(Ordering::Acquire)
        {
    ...
    pub fn preflight_configuration(
        ...
        let config = LocalNodeExecutorConfig {
            ...
            max_event_loop_unresponsive: LOCAL_NODE_EXECUTOR_POOL_POLICIES
                ...
                .and_then(|policy| policy.max_event_loop_unresponsive_seconds)
                .map(Duration::from_secs),
            max_old_space_size_mib: local_node_executor_pool_old_space_size_mib("default"),
            max_rss_bytes: u64::try_from(local_node_executor_pool_rss_bytes("default"))
                .context("Local Node executor RSS threshold does not fit u64")?,
            memory_pressure,
            memory_pressure_min_rss_bytes: u64::try_from(
                local_node_executor_pool_memory_pressure_min_rss_bytes("default"),
            )
            .context("Local Node executor cgroup-pressure RSS threshold does not fit u64")?,
        ...
        config.validate()?;
        for pool_name in LOCAL_NODE_EXECUTOR_POOL_POLICIES.keys() {
            config
                .clone()
                .with_pool_name(Arc::from(pool_name.as_str()))?
                .validate()?;
        }
        Ok(config)
    }
    ...
    pub async fn new_with_configuration(
        ...
        let activity = Arc::new(ExecutorPoolActivity {
            ...
            waiting_requests: StdMutex::new(0),
            active_requests: StdMutex::new(0),
        });
        let executor = Self {
    ...
    async fn run_hot_replacement(
        ...
        if let Some(descriptor) = descriptor {
            ...
            let preparation = candidate.prepare_package(
                &preparation_descriptor,
                if candidate.resident_fingerprint.is_some() {
                    PreparationMode::Resident
                } else {
                    PreparationMode::Warm
                },
                "candidate_readiness",
                config.node_process_timeout,
            );
        ...
        let promoted = {
                ...
                && cleanup.promote_to_draining(expected.clone())
            {
                ...
                // Candidate preparation can already retain packages. Drop the
                // old generation's values until this generation's first
                // validated health snapshot measures its state.
                crate::metrics::clear_local_node_package_state(&config.pool_name);
                true
            } else {
                false
            }
    ...
    async fn acquire_inner(
        ...
        resident_fingerprint: Option<ResidentGenerationFingerprint>,
        preparation_descriptor: Option<PreparationDescriptor>,
        generation_access: ResidentGenerationAccess,
        ...
        loop {
            ...
            match self
                .acquire_existing_inner_with_descriptor(
                    ...
                    generation_access.allows_fingerprint_replacement(),
                )
                .await?
            {
                ...
                InnerAcquisition::Draining(inner) => {
                    anyhow::ensure!(
                        generation_access.allows_cold_start_after_generation_loss(),
                        "Node executor request lost its required resident generation"
                    );
                    inner.wait_until_retired().await?;
                },
        ...
        anyhow::ensure!(
            generation_access.allows_empty_cold_start(),
            "Node executor request has no required resident generation"
        );
        ...
        let startup_guard = self.startup_lock.clone().lock_owned().await;
        loop {
            let transition_changed = self.transition_changed.notified();
            tokio::pin!(transition_changed);
            transition_changed.as_mut().enable();
        ...
        let cold_startup_owner = Arc::new(ColdStartupOwner::new(
            self.config.pool_name.clone(),
            generation,
            startup_guard,
        ));
        ...
        // Startup health establishes protocol support but is not exported.
        // Leave package gauges absent until the watchdog publishes this
        // generation's first validated snapshot rather than inventing zeros.
        crate::metrics::clear_local_node_package_state(&self.config.pool_name);
        ...
        // Admission is now published under the generation-state lock, so the
        // state owns process capacity instead of the cold-start cleanup owner.
        cold_startup_owner.finish();
    ...
    async fn acquire_existing_inner_with_descriptor(
        ...
        resident_fingerprint: Option<ResidentGenerationFingerprint>,
        preparation_descriptor: Option<PreparationDescriptor>,
        allow_fingerprint_replacement: bool,
        ...
        if let Some(inner) = &state.inner {
            ...
            if resident_fingerprint.is_some() && !fingerprint_matches {
                ...
                if !allow_fingerprint_replacement {
                    anyhow::bail!("Node executor request target changed during concurrent cutover");
                }
                drop(state);
                let status = self
    ...
    async fn acquire_existing_inner(
        ...
        self.acquire_existing_inner_with_descriptor(
            request_metadata,
            resident_fingerprint,
            None,
            true,
    ...
    async fn response_stream(mut response: reqwest::Response, deadline: tokio::time::Instant) {
        anyhow::ensure!(
            response
                .content_length()
                .is_none_or(|length| length <= MAX_STREAMED_RESPONSE_BYTES as u64),
    ...
    async fn observe_http_error_body(
        ...
        loop {
            ...
            response_bytes = response_bytes
                .checked_add(chunk.len())
                .filter(|size| *size <= MAX_STREAMED_RESPONSE_BYTES)
                .context("Local Node executor response exceeded size limit")?;
        }
    }
                ...
                .is_some_and(|current| Arc::ptr_eq(current, expected))
            {
                ...
                assert!(
                    expected
                        ...
                        .replace(reason)
                        .is_none(),
                    "Local Node generation retirement reason was published twice"
                );
                ...
                // The generation is no longer current, but its direct child
                // can still retain packages until termination completes. Drop
                // generation-local observations instead of publishing an
                // unmeasured empty cache during that interval.
                crate::metrics::clear_local_node_package_state(&expected.pool_name);
                ...
                .is_some_and(|retiring| Arc::ptr_eq(retiring, expected))
            {
                // A concurrent owner already closed admission and owns this
                // exact child. Join its terminal result so request admission
                // and system-operation ownership cannot end before reaping.
                (false, true)
            } else {
                (false, false)
            }
        };
        if joined_retirement {
            expected.wait_until_retired().await?;
            return Ok(false);
        }
        if !retired {
    ...
    async fn terminate_retiring_inner_state(
        ...
        let result = if expected.termination_confirmed.load(Ordering::Acquire) {
            // A prior attempt already reaped the child but lost its task before
            // it could clear state.retiring. The state publication retry must
            // not call terminate again and misclassify confirmed cleanup.
            Ok(None)
        } else {
            expected.terminate().await.map(Some)
        };
        match &result {
            Ok(Some(observation)) => {
                expected
    ...
    async fn finish_retiring_inner_state(
        state: &Arc<Mutex<LocalNodeExecutorState>>,
        expected: &Arc<InnerLocalNodeExecutor>,
        reason: GenerationRetirementReason,
    ) -> anyhow::Result<()> {
        loop {
                ...
                .is_ok()
            {
                ...
                return Self::terminate_retiring_inner_state(state, expected, reason).await;
            }
    ...
    async fn finish_retiring_inner_for_shutdown(
        ...
        Self::finish_retiring_inner_state(
            state,
            expected,
    ...
    async fn watch_generation_checks(
        ...
        loop {
                ...
                tokio::select! {
                    ...
                    health = health_check.as_mut(), if health_observation.is_none() => {
                        let success = health.as_ref().is_some_and(|health| {
                            health.is_valid_generation_observation(
                                &previous_package_stats,
                                &previous_stack_stats,
                                expected.runtime_stats_supported,
                                expected.resident_source_owners_supported,
                            )
                        });
                        if success {
            ...
            if let Some(reason) = retirement_reason {
                ...
                if matches!(config.role, LocalNodeExecutorRole::System) {
                    // The internal system slot recycles cold between
                    // operations. It never consumes the application surge
                    // allowance or overlaps a healthy replacement child.
                    let diagnostics = GenerationRetirementDiagnostics::proactive(reason);
                    if Self::start_draining_inner_state(&state, &expected, reason).await
                        && Self::finish_draining_inner_state(&state, &expected, diagnostics)
                            .await
                            .is_err()
                    {
                        tracing::error!(
                            pool_name = %expected.pool_name,
                            generation = expected.generation,
                            "Failed to recycle local Node system generation"
                        );
                    }
                    return;
                }
                ...
                // must not silently downgrade to health-only readiness. With
                // no signer available to refresh that authority, retire this
                // generation and let the next request cold-start with fresh
                // authority instead of retaining it past the generation-age
                // or package/RSS retirement bound.
                match descriptor {
                    Some(descriptor) if descriptor.is_expired() => {
                                ...
                                .is_err()
                        {
                            tracing::error!(
                                ...
                                generation = expected.generation,
                                reason = reason.as_str(),
                                "Failed to retire local Node executor with expired package authority"
                            );
                        }
                        return;
                    },
    ...
    fn record_node_health_metrics(
        ...
        crate::metrics::set_local_node_package_state(
            ...
            package.resident_source_owners,
            package.retained_external_packages,
            package.retained_external_bytes,
            stack.registered_roots,
        );
        let first_available_sequence = package.package_stage_observations_dropped.saturating_add(1);
        let first_unobserved_sequence = previous_package_stats
            .package_stage_observation_sequence
            .saturating_add(1);
        crate::metrics::log_local_node_package_stage_observations_dropped(
            &inner.pool_name,
            first_available_sequence.saturating_sub(first_unobserved_sequence),
        );
        for observation in package
            .package_stage_observations
            .iter()
            .filter(|observation| {
                observation.sequence() > previous_package_stats.package_stage_observation_sequence
            })
        {
            let elapsed = Duration::from_secs_f64(observation.duration_ms() / 1000.0);
            match observation {
                NodePackageStageObservation::Acquire {
                    request_kind,
                    cache_result,
                    outcome,
                    ..
                } => crate::metrics::log_local_node_package_acquire(
                    &inner.pool_name,
                    request_kind.as_str(),
                    cache_result.as_str(),
                    elapsed,
                ...
                NodePackageStageObservation::Validation {
                    package_kind,
                    phase,
                    outcome,
                    ..
                } => crate::metrics::log_local_node_package_validation(
                    &inner.pool_name,
                    package_kind.as_str(),
                    phase.as_str(),
                    elapsed,
                ...
                NodePackageStageObservation::Cleanup {
                    package_kind,
                    outcome,
                    ..
                } => crate::metrics::log_local_node_package_cleanup(
                    &inner.pool_name,
                    package_kind.as_str(),
                    elapsed,
                    outcome.as_str(),
...
impl LocalNodeExecutor {
    async fn prepare_resident_package(
        &self,
        inner: &Arc<InnerLocalNodeExecutor>,
        descriptor: &PreparationDescriptor,
        ...
        // The selected application generation retains this package lease for
        // its complete child lifetime. A repeated identity confirmation does
        // not depend on the signed URL remaining fresh.
        let preparation = inner.prepare_package(
            descriptor,
            PreparationMode::Resident,
            purpose,
            self.config.node_process_timeout,
        );
        tokio::pin!(preparation);
        ...
        let preparation_result = tokio::select! {
            biased;
            _ = &mut hard_shutdown => Err((
                "generation_lost_before_start",
                anyhow::anyhow!("Local Node executor shut down before execution start"),
            )),
            _ = &mut unavailable => Err((
                "generation_lost_before_start",
                anyhow::anyhow!(
                    "Local Node executor generation became unavailable before execution start"
                ),
            )),
            result = &mut preparation => result.map_err(|error| {
                (
                    "preparation_failed_before_start",
                    error.context("Node package preparation failed before execution admission"),
                )
            }),
        };
        if let Err((outcome, error)) = preparation_result {
            request_guard.set_outcome(outcome);
            return Err(error);
        }
        // A cold fingerprinted child is published before preparation so its
        // exact request guard can participate in promotion ordering. Wake
        // router readiness waiters only after retained ownership is real.
        self.transition_changed.notify_waiters();
        Ok(())
    }

    async fn wait_for_execution_start(
        &self,
        inner: &Arc<InnerLocalNodeExecutor>,
        ...
        let start_result = tokio::select! {
            ...
            result = &mut start => result.map_err(|error| ("canceled_before_start", error)),
        };
        if let Err((outcome, error)) = start_result {
            request_guard.set_outcome(outcome);
            return Err(error);
        }
        Ok(())
    ...
    async fn prepare_and_wait_for_execution_start(
        ...
        self.prepare_resident_package(inner, descriptor, request_guard, "scheduled_readiness")
            .await?;
        self.wait_for_execution_start(inner, start_gate, request_guard)
            .await
    }
    ...
    pub(crate) async fn invoke_with_fingerprint(
        ...
        let resident_request = resident_fingerprint.is_some();
        ...
        if resident_request {
            let descriptor = preparation_descriptor
                .as_ref()
                .context("Resident Node execute request is missing its package descriptor")?;
            self.prepare_resident_package(
                &inner,
                descriptor,
                &mut request_guard,
                if function_execution_start.is_some() {
                    "scheduled_readiness"
                } else if created {
                    "resident_startup"
                } else {
        ...
        if let Some(start_gate) = function_execution_start {
            self.wait_for_execution_start(&inner, start_gate, &mut request_guard)
                .await?;
        }

        request_guard.start_request_timer();
        ...
        let response = match response_result {
            Err(_) => {
                ...
                .await
                .inspect_err(|_| request_guard.set_outcome("internal_error"))?;
        ...
        if let Err(e) = response.error_for_status_ref() {
            ...
            if status == reqwest::StatusCode::CONFLICT {
                // The system child uses 409 only when an earlier operation is
                // still active. Retire the exact process instead of treating
                // the rejection body as terminal child-work observation.
                self.retire_inner_if_current(
                    &inner,
                    GenerationRetirementDiagnostics::request(
                        GenerationRetirementReason::ResponseStreamError,
                        request_kind,
                        "response_body",
                        "not_applicable",
                    ),
                ...
                anyhow::bail!("Node executor server returned HTTP {}", status.as_u16());
            }
            match Self::observe_http_error_body(response, request_deadline).await {
                Ok(true) => {},
                Ok(false) => {
                    self.retire_inner_if_current(
                        &inner,
                ...
                Err(error) => {
                    ...
                    anyhow::bail!("Failed to consume local Node executor error response");
                },
            }
...
fn test_inner_with_owner(
    ...
    let resident_package_ready = resident_fingerprint.is_some();
    Arc::new(InnerLocalNodeExecutor {
        generation,
        pool_name: pool_name.clone(),
        resident_fingerprint,
        resident_package_ready: AtomicBool::new(resident_package_ready),
        activity,
        pid,
        started_at: Instant::now(),
        runtime_stats_supported: false,
        resident_source_owners_supported: false,
        active_requests: AtomicUsize::new(0),
        retirement_requested: AtomicBool::new(false),
        retirement_reason: StdMutex::new(None),
        idle: Notify::new(),
...
mod tests {
    ...
    #[tokio::test]
    async fn resident_preparation_rejects_legacy_success_without_residency() {
        let (generation, server) =
            test_inner_with_preparation_response(r#"{"type":"success"}"#).await;
        let descriptor = PreparationDescriptor {
            source_package: test_source_package(),
        };
        ...
        assert!(error
            .to_string()
            .contains("Invalid local Node executor preparation response"));
        server.await.unwrap();
        generation.terminate().await.unwrap();
    }
    ...
    async fn execution_start_ready_holds_exact_generation_admission() {
        let (inner, server) = test_inner_with_preparation_response(
            r#"{"type":"success","residency":"retained","result":"initialized"}"#,
        )
        .await;
        ...
        assert!(inner.resident_package_ready.load(Ordering::Acquire));
        ...
        assert_eq!(outcome, "canceled_before_start");
    ...
    #[tokio::test]
    async fn hard_unavailability_preserves_generation_loss_during_preparation() {
        let socket_dir = TempDir::new().unwrap();
        let socket_path = socket_dir.path().join("executor.sock");
        let listener = UnixListener::bind(&socket_path).unwrap();
        let (request_received_sender, request_received_receiver) = oneshot::channel();
        ...
        assert_eq!(outcome, "generation_lost_before_start");
        assert_eq!(inner.active_requests.load(Ordering::Acquire), 0);
    ...
    async fn package_preparation_failure_happens_before_ready() {
        ...
        assert_eq!(outcome, "preparation_failed_before_start");
        assert_eq!(inner.active_requests.load(Ordering::Acquire), 0);
        server.await.unwrap();
        inner.terminate().await.unwrap();
    }
    ...
    #[tokio::test]
    async fn expired_package_authority_allows_retained_resident_reuse_before_ready() {
        let (inner, server) = test_inner_with_preparation_response(
            r#"{"type":"success","residency":"retained","result":"reused"}"#,
        )
        .await;
        let (executor, _) = test_executor(inner.clone(), test_config());
        let guard = ActiveRequestGuard::new(inner.clone(), test_request_metadata());
    ...
    fn health_runtime_stats_require_a_complete_pair_and_valid_counters() {
        ...
        assert!(!upstream.resident_source_owners_supported());

        let current = NodeExecutorHealth {
            status: "ok".to_string(),
            package_cache: Some(NodePackageCacheStats {
                resident_source_owners: Some(1),
        ...
        assert!(current.resident_source_owners_supported());
        assert_eq!(
        ...
        assert!(current.is_valid_generation_observation(
            &NodePackageCacheStats::default(),
            &NodeStackTraceStats::default(),
            true,
            true,
        ...
        let earlier_package: NodePackageCacheStats =
            serde_json::from_value(earlier_package_value.clone()).unwrap();
        assert_eq!(earlier_package.resident_source_owners, None);
        let earlier_package_health = NodeExecutorHealth {
            status: "ok".to_string(),
            package_cache: Some(earlier_package),
            stack_trace: Some(NodeStackTraceStats::default()),
        };
        assert!(!earlier_package_health.resident_source_owners_supported());
        assert_eq!(
            earlier_package_health.valid_runtime_stats_support(
                &NodePackageCacheStats::default(),
                &NodeStackTraceStats::default(),
            ),
            Some(true)
        );
        assert!(earlier_package_health.is_valid_generation_observation(
            &NodePackageCacheStats::default(),
            &NodeStackTraceStats::default(),
            true,
            false,
        ...
        earlier_package_value["residentSourceOwners"] = JsonValue::Null;
        assert!(serde_json::from_value::<NodePackageCacheStats>(earlier_package_value).is_err());

        let observed_package = NodePackageCacheStats {
            package_stage_observation_sequence: 1,
            package_stage_observations: vec![NodePackageStageObservation::Acquire {
                sequence: 1,
                request_kind: NodePackageRequestKind::Prepare,
                cache_result: NodePackageCacheResult::NewMaterialization,
                outcome: NodePackageStageOutcome::Success,
                duration_ms: 12.5,
            }],
            ..NodePackageCacheStats::default()
        };
        assert!(observed_package
            .package_stage_observations_are_valid(&NodePackageCacheStats::default()));
        let invalid_observation_gap = NodePackageCacheStats {
            package_stage_observation_sequence: 2,
            package_stage_observations: vec![NodePackageStageObservation::Cleanup {
                sequence: 2,
                package_kind: NodePackageKind::Source,
                outcome: NodePackageStageOutcome::Success,
                duration_ms: 1.0,
            }],
            ..NodePackageCacheStats::default()
        };
        assert!(!invalid_observation_gap
            .package_stage_observations_are_valid(&NodePackageCacheStats::default()));
        ...
        let unrepresentable_duration = NodeExecutorHealth {
            status: "ok".to_string(),
            package_cache: Some(NodePackageCacheStats::default()),
            stack_trace: Some(NodeStackTraceStats {
                duration_ms: f64::MAX,
        ...
        assert_eq!(
            unrepresentable_duration.valid_runtime_stats_support(
                &NodePackageCacheStats::default(),
                &NodeStackTraceStats::default(),
            ),
            None
    ...
    #[test]
    fn package_stage_health_snapshot_is_bounded_and_strict() {
        let observations = (1..=MAX_PACKAGE_STAGE_OBSERVATIONS)
            .map(|sequence| {
                serde_json::json!({
                    "sequence": sequence,
                    "stage": "acquire",
                    "requestKind": "analyze",
                    "cacheResult": "joined_materialization",
                    "outcome": "failure",
                    "durationMs": 1.0,
                })
            })
        ...
        let payload = serde_json::json!({
            "status": "ok",
            "packageCache": {
                "importedSourcePackages": u64::MAX,
                "retainedSourcePackages": u64::MAX,
                "retainedSourceBytes": u64::MAX,
                "activeSourceOwners": u64::MAX,
                "residentSourceOwners": u64::MAX,
                "retainedExternalPackages": u64::MAX,
                "retainedExternalBytes": u64::MAX,
                "sourceHits": u64::MAX,
                "sourcePublishes": u64::MAX,
                "sourceRetirements": u64::MAX,
                "sourceFailedPublications": u64::MAX,
                "externalHits": u64::MAX,
                "externalPublishes": u64::MAX,
                "externalRetirements": u64::MAX,
                "externalFailedPublications": u64::MAX,
                "packageStageObservationSequence": MAX_PACKAGE_STAGE_OBSERVATIONS,
                "packageStageObservationsDropped": 0,
                "packageStageObservations": observations,
            },
            "stackTrace": {
                "registeredRoots": u64::MAX,
                "invocations": u64::MAX,
                "framesProcessed": u64::MAX,
                "durationMs": 1.0,
            },
        });
        let body = serde_json::to_vec(&payload).unwrap();
        assert!(body.len() <= MAX_HEALTH_RESPONSE_BYTES);
        let health: NodeExecutorHealth = serde_json::from_slice(&body).unwrap();
        assert!(health.is_valid_generation_observation(
            &NodePackageCacheStats::default(),
            &NodeStackTraceStats::default(),
            true,
            true,
        ...
        let observation = |sequence| NodePackageStageObservation::Cleanup {
            sequence,
            package_kind: NodePackageKind::Source,
            outcome: NodePackageStageOutcome::Success,
            duration_ms: 1.0,
        ...
        let empty_package_stats = NodePackageCacheStats::default();
        let overwritten = NodePackageCacheStats {
            package_stage_observation_sequence: 300,
            package_stage_observations_dropped: 44,
            package_stage_observations: (45..=300).map(observation).collect(),
            ..NodePackageCacheStats::default()
        };
        assert!(overwritten.package_stage_observations_are_valid(&empty_package_stats));

        let duplicate = NodePackageCacheStats {
            package_stage_observation_sequence: 2,
            package_stage_observations: vec![observation(1), observation(1)],
            ..NodePackageCacheStats::default()
        };
        assert!(!duplicate.package_stage_observations_are_valid(&empty_package_stats));

        let missing_retained_suffix = NodePackageCacheStats {
            package_stage_observation_sequence: 1,
            package_stage_observations_dropped: 1,
            package_stage_observations: Vec::new(),
            ..NodePackageCacheStats::default()
        };
        let missing_suffix_is_valid =
            missing_retained_suffix.package_stage_observations_are_valid(&empty_package_stats);
        assert!(!missing_suffix_is_valid);

        let oversized = NodePackageCacheStats {
            package_stage_observation_sequence: (MAX_PACKAGE_STAGE_OBSERVATIONS + 1) as u64,
            package_stage_observations: (1..=(MAX_PACKAGE_STAGE_OBSERVATIONS + 1))
                .map(|sequence| observation(sequence as u64))
                .collect(),
            ..NodePackageCacheStats::default()
        };
        assert!(!oversized.package_stage_observations_are_valid(&empty_package_stats));

        let overflow = NodePackageCacheStats {
            package_stage_observation_sequence: u64::MAX,
            package_stage_observations_dropped: u64::MAX,
            ..NodePackageCacheStats::default()
        };
        assert!(!overflow.package_stage_observations_are_valid(&empty_package_stats));

        let reset = NodePackageCacheStats {
            package_stage_observation_sequence: 1,
            package_stage_observations: vec![observation(1)],
            ..NodePackageCacheStats::default()
        };
        let previous = NodePackageCacheStats {
            package_stage_observation_sequence: 2,
            package_stage_observations: vec![observation(1), observation(2)],
            ..NodePackageCacheStats::default()
        };
        assert!(previous.package_stage_observations_are_valid(&empty_package_stats));
        assert!(!reset.package_stage_observations_are_valid(&previous));

        let invalid_duration = NodePackageCacheStats {
            package_stage_observation_sequence: 1,
            package_stage_observations: vec![NodePackageStageObservation::Cleanup {
                sequence: 1,
                package_kind: NodePackageKind::Source,
    ...
    #[test]
    fn config_accepts_a_pool_sized_memory_policy() {
        let mut config = test_config();
        config.max_old_space_size_mib = 512;
        config.max_rss_bytes = 1024 * MIB_BYTES;
        config.memory_pressure_min_rss_bytes = 768 * MIB_BYTES;
        config.validate().unwrap();
    }

    #[test]
    fn system_role_has_a_distinct_pool_and_shared_surge_coordinator() {
        let application = test_config();
        let application_surge = application.surge_coordinator();
        let system = application.into_system_role().unwrap();

        assert!(matches!(system.role, LocalNodeExecutorRole::System));
        assert_eq!(system.pool_name.as_ref(), "_system");
        assert_eq!(
            system.max_rss_bytes,
            u64::try_from(*LOCAL_NODE_EXECUTOR_MAX_RSS_BYTES).unwrap()
        );
        assert_eq!(
            system.max_old_space_size_mib,
            *LOCAL_NODE_EXECUTOR_MAX_OLD_SPACE_SIZE_MIB
        );
        assert_eq!(
        ...
        assert!(Arc::ptr_eq(&application_surge, &system.surge_coordinator()));
    }
    ...
    fn default_override_does_not_change_system_memory_policy() {
        ...
        let mut application = test_config();
        application.max_old_space_size_mib = 1;
        application.memory_pressure_min_rss_bytes = global_rss_bytes;
...
"#,
    ...
    async fn dropped_unpublished_child_retains_capacity_and_tempdir_until_reaped() {
        let source_dir = TempDir::new().unwrap();
        ...
        let startup_guard = startup_lock.clone().lock_owned().await;
        let startup_owner = Arc::new(ColdStartupOwner::new(
            Arc::from("default"),
            1,
            startup_guard,
        ));
        let server_handle = ManagedChild::new(
            Arc::from("default"),
            1,
            server_handle,
            source_dir,
            Some(startup_owner.clone()),
        );
        ...
        assert!(startup_lock.try_lock().is_err());
        tokio::time::timeout(Duration::from_secs(1), async {
        ...
        .await
        .unwrap();
        assert!(startup_lock.try_lock().is_ok());
    }
    ...
    #[tokio::test]
    async fn required_resident_generation_does_not_cold_start_after_retirement() {
        let fingerprint = test_fingerprint(b"stale");
        ...
        let result = executor
            .acquire_inner(
                ...
                Some(fingerprint),
                None,
                ResidentGenerationAccess::RequireExisting,
            )
            .await;
        let error = match result {
            Ok(_) => panic!("A stale request recreated a retired resident generation"),
            Err(error) => error,
        };
        assert!(error
            .to_string()
            .contains("lost its required resident generation"));
        assert!(state
            .lock()
            .await
            .inner
    ...
    #[tokio::test]
    async fn routed_target_does_not_retarget_a_concurrent_generation() {
        let current_fingerprint = test_fingerprint(b"current");
        let requested_fingerprint = test_fingerprint(b"requested");
        ...
        let result = executor
            .acquire_inner(
                ...
                None,
                ResidentGenerationAccess::AllowColdStartWithoutRetarget,
            )
            .await;
        let error = match result {
            Ok(_) => panic!("A routed request retargeted the concurrent generation"),
            Err(error) => error,
        };
        assert!(error
            .to_string()
            .contains("target changed during concurrent cutover"));
        assert!(state
            .lock()
            .await
            .inner
    ...
    #[tokio::test]
    async fn routed_request_does_not_cold_start_after_generation_admission_closes() {
        let fingerprint = test_fingerprint(b"requested");
        let generation = test_inner_with_fingerprint(1, fingerprint.clone()).await;
        generation
            .retirement_requested
            .store(true, Ordering::Release);
        ...
        let result = executor
            ...
            .await;
        let error = match result {
            Ok(_) => panic!("A routed request cold-started after admission closed"),
            Err(error) => error,
        };
        assert!(error
            .to_string()
    ...
    #[tokio::test]
    async fn fingerprint_does_not_prove_resident_package_readiness() {
        let fingerprint = test_fingerprint(b"first");
        let generation = test_inner_with_fingerprint(1, fingerprint.clone()).await;
        generation
        ...
        assert!(!executor.resident_fingerprint_matches(&fingerprint).await);
        let readiness_executor = executor.clone();
        let readiness_fingerprint = fingerprint.clone();
        let readiness = tokio::spawn(async move {
            readiness_executor
                .wait_for_resident_package_and_environment_ready(&readiness_fingerprint)
                .await
        });
        tokio::task::yield_now().await;
        ...
        generation
            .resident_package_ready
            .store(true, Ordering::Release);
        executor.transition_changed.notify_waiters();
        tokio::time::timeout(Duration::from_secs(1), readiness)
            .await
            .expect("Resident readiness did not observe retained package ownership")
            .unwrap()
            .unwrap();
        assert!(executor.resident_fingerprint_matches(&fingerprint).await);
        generation.terminate().await.unwrap();
    ...
    async fn resident_readiness_fails_when_target_generation_becomes_unavailable() {
        ...
        tokio::task::yield_now().await;
        generation.mark_execution_unavailable();
        // Model hard retirement's state publication: it closes admission and
        // removes the current generation before its cleanup task reaps it.
        {
            let mut state = state.lock().await;
            assert!(state
                .inner
                .take()
                .is_some_and(|current| Arc::ptr_eq(&current, &generation)));
            state.retiring = Some(generation.clone());
        }

        let error = tokio::time::timeout(Duration::from_secs(1), readiness)
            .await
            .expect("Resident readiness remained pending after generation loss")
            .unwrap()
            .expect_err("Unavailable resident generation was reported as ready");
        assert_eq!(
            error.to_string(),
            "Local Node executor target generation became unavailable before readiness"
        );
    ...
    async fn matching_package_reuses_generation_across_topology_watermarks() {
        ...
        assert!(
            executor
                .resident_fingerprint_matches_snapshot(&newer_snapshot)
                .await
        );
    ...
    #[tokio::test]
    async fn newer_resident_generation_does_not_satisfy_an_older_snapshot() {
        let mut resident = test_fingerprint(b"first");
        resident.topology_version = Timestamp::try_from(2_u64).unwrap();
        let generation = test_inner_with_fingerprint(1, resident).await;
        let (executor, _) = test_executor(generation.clone(), test_config());
        let mut older_snapshot = test_fingerprint(b"first");
        older_snapshot.topology_version = Timestamp::try_from(1_u64).unwrap();

        assert!(
            !executor
                .resident_fingerprint_matches_snapshot(&older_snapshot)
                .await
        );
        generation.terminate().await.unwrap();
    }
    ...
    async fn deployment_cutover_reuses_matching_package_across_topology_watermarks() {
        ...
        let outcome = executor
            .replace_for_deployment(target_fingerprint, test_source_package(), permit)
            .await
            .unwrap();
    ...
    async fn retirement_reaps_child_after_retiring_caller_is_canceled() {
        ...
        let joined_generation = generation.clone();
        let mut joined_retirement = tokio::spawn(async move {
            LocalNodeExecutor::retire_inner_state(
                &joined_state,
                &joined_generation,
                test_request_retirement(GenerationRetirementReason::ConnectionError),
            )
            .await
        });
        assert!(
            tokio::time::timeout(Duration::from_millis(10), &mut joined_retirement)
                .await
                .is_err()
        );
        ...
        assert!(!joined_retirement.await.unwrap().unwrap());
    ...
    #[tokio::test]
    async fn response_protocol_failure_retires_generation_before_eof() {
        let socket_dir = TempDir::new().unwrap();
        let socket_path = socket_dir.path().join("executor.sock");
        let listener = UnixListener::bind(&socket_path).unwrap();
        let server_task = tokio::spawn(async move {
    ...
    #[tokio::test]
    async fn malformed_terminal_payload_retires_and_reaps_selected_generation() {
        for (payload, expected_error) in [
            (
                r#"{"type":"invalid"}"#,
                "Node executor returned an invalid response type",
            ),
            (
                r#"{"type":"success","exitingProcess":"yes"}"#,
                "Node executor returned an invalid exitingProcess response field",
            ),
        ] {
            let socket_dir = TempDir::new().unwrap();
    ...
    #[tokio::test]
    async fn dropped_log_receiver_does_not_end_request_ownership() {
        let socket_dir = TempDir::new().unwrap();
        let socket_path = socket_dir.path().join("executor.sock");
        let listener = UnixListener::bind(&socket_path).unwrap();
        let server_task = tokio::spawn(async move {
            ...
            socket
                .write_all(
                    b"HTTP/1.1 200 OK\r\nContent-Type: application/x-ndjson\r\nConnection: close\r\n\r\n{\"kind\":\"LogLine\",\"data\":{\"messages\":[\"test\"],\"isTruncated\":false,\"timestamp\":0,\"level\":\"INFO\"}}\n{\"type\":\"success\"}\n",
                )
                .await
                .unwrap();
        });
        ...
        assert_eq!(response.response, serde_json::json!({ "type": "success" }));
        assert!(state
            .lock()
            .await
            .inner
    ...
    #[tokio::test]
    async fn http_rejection_retains_ownership_until_terminal_cleanup() {
        for (status, returns_timeout) in
            [("409 Conflict", false), ("500 Internal Server Error", true)]
        {
            let socket_dir = TempDir::new().unwrap();
            let socket_path = socket_dir.path().join("executor.sock");
            let listener = UnixListener::bind(&socket_path).unwrap();
            ...
            if returns_timeout {
                assert_eq!(
                    ...
                    EXECUTE_TIMEOUT_RESPONSE_JSON.clone()
                );
            } else {
                assert!(result.is_err());
            }
    ...
    #[tokio::test]
    async fn retirement_retry_preserves_confirmed_reap() {
        let inner = test_inner(1).await;
        inner.terminate().await.unwrap();
        inner.termination_confirmed.store(true, Ordering::Release);
        inner.retirement_failed.store(true, Ordering::Release);
        let state = Arc::new(Mutex::new(LocalNodeExecutorState {
            inner: None,
            retiring: Some(inner.clone()),
            hot_transition: None,
            replacement_for_generation: Some(inner.generation),
            next_generation: inner.generation,
            next_transition: 0,
        }));

        LocalNodeExecutor::finish_retiring_inner_state(
            &state,
            &inner,
            GenerationRetirementReason::ConnectionError,
        )
