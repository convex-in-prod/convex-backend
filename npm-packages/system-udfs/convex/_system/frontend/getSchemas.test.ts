...
type SchemaState =
  "pending" | "validated" | "active" | "failed" | "overwritten";
...
function databaseReader(
  ...
  const query = (tableName: string) => ({
    withIndex(
      ...
      return {
        async collect() {
          expect(tableName).toBe("_schema_validations");
          expect(indexName).toBe("by_schema_id_and_table_name");
          if (!equality) {
            throw new Error("Expected a schema equality");
          }
          expect(equality.field).toBe("schemaId");
          return [];
        },
        async unique() {
          if (!equality) {
