...
const MAX_PACKAGE_STAGE_OBSERVATIONS = 256;

type PackageRequestKind = "execute" | "analyze" | "prepare";
type PackageCacheResult =
  | "current_hit"
  | "joined_materialization"
  | "new_materialization"
  | "retirement_wait";
const PACKAGE_CACHE_RESULT_PRIORITY: Record<PackageCacheResult, number> = {
  current_hit: 0,
  retirement_wait: 1,
  joined_materialization: 2,
  new_materialization: 3,
};
type PackageKind = "source" | "external";
type PackageValidationPhase = "staged" | "prebuilt";
type PackageStageOutcome = "success" | "failure";
type PackageStageObservation =
  | {
      sequence: number;
      stage: "acquire";
      requestKind: PackageRequestKind;
      cacheResult: PackageCacheResult;
      outcome: PackageStageOutcome;
      durationMs: number;
    }
  | {
      sequence: number;
...
type PackageStageObservationInput = PackageStageObservation extends infer T
  ? T extends PackageStageObservation
    ? Omit<T, "sequence">
    : never
  : never;

let packageStageObservationSequence = 0;
let packageStageObservationsDropped = 0;
const packageStageObservations: PackageStageObservation[] = [];

function recordPackageStageObservation(
  observation: PackageStageObservationInput,
) {
  packageStageObservationSequence += 1;
  packageStageObservations.push({
    ...observation,
    sequence: packageStageObservationSequence,
  } as PackageStageObservation);
  if (packageStageObservations.length > MAX_PACKAGE_STAGE_OBSERVATIONS) {
    packageStageObservations.shift();
    packageStageObservationsDropped += 1;
  }
}

async function observePackageValidation<T>(
  packageKind: PackageKind,
  phase: PackageValidationPhase,
  operation: () => Promise<T>,
): Promise<T> {
...
function observePackageCleanup(
  packageKind: PackageKind,
...
async function maybeDownloadAndLinkPackagesInner(
  sourcePackage: SourcePackage,
  observeCacheResult?: (result: PackageCacheResult) => void,
): Promise<LocalSourcePackage> {
  ...
  for (;;) {
    ...
    if (retirement !== undefined) {
      observeCacheResult?.("retirement_wait");
      await retirement;
    }
    ...
    if (local !== undefined) {
      observeCacheResult?.("current_hit");
      validatePackageChecksum(
        local.archiveSha256,
        sourcePackage.bundled_source,
      );
      validateExternalDepsMatch(local, sourcePackage.external_deps);
      // Publication validated this immutable package before installing the
      // current map entry. Reuse checks identity only; unsupported external
      // filesystem mutation requires a fresh executor generation.
    ...
    if (inFlight !== undefined) {
      observeCacheResult?.("joined_materialization");
      validatePackageChecksum(
    ...
    observeCacheResult?.("new_materialization");
...
async function maybeDownloadExternalPackage(
  ...
  for (;;) {
    ...
    if (externalDeps !== undefined) {
      ...
      // Source packages retain symlinks to an external package that was
      // validated before publication. Current-entry reuse is identity-only.
...
async function downloadExternalPackage(
  ...
  try {
    ...
    await validateLocalExternalPackage({ dir: stagingDir }, "staged");
...
export type SourcePackageOwner = "request" | "resident";

function sourcePackageOwnerCount(localPackage: LocalSourcePackage): number {
  return localPackage.activeOwners + localPackage.residentOwners;
}

export async function acquireSourcePackage(
  sourcePackage: SourcePackage,
  owner: SourcePackageOwner = "request",
  requestKind: PackageRequestKind = "execute",
): Promise<SourcePackageLease> {
  const started = performance.now();
  let cacheResult: PackageCacheResult = "current_hit";
  let acquisitionRecorded = false;
  const recordAcquisition = (outcome: PackageStageOutcome) => {
    if (acquisitionRecorded) {
      throw new PackageCacheError(
        "Source package acquisition telemetry was recorded twice",
      );
    }
  ...
  try {
    for (;;) {
      localPackage = await maybeDownloadAndLinkPackagesInner(
        sourcePackage,
        (result) => {
          // Ownership can lose a retirement race and repeat lookup. Retain the
          // most substantial work from the complete acquisition, not only the
          // final lookup that acquired the current map entry.
          if (
            PACKAGE_CACHE_RESULT_PRIORITY[result] >
            PACKAGE_CACHE_RESULT_PRIORITY[cacheResult]
          ) {
            cacheResult = result;
          }
        },
  ...
  return {
    ...
    release: async () => {
      ...
      released = true;
      if (owner === "resident") {
        if (localPackage.residentOwners <= 0) {
          throw new PackageCacheError(
            "Source package resident-owner count is invalid",
          );
        }
        localPackage.residentOwners -= 1;
      } else {
...
export function getPackageCacheStats() {
  ...
  return {
    ...
    residentSourceOwners: sourcePackages.reduce(
      (total, localPackage) => total + localPackage.residentOwners,
      0,
    ),
    ...
    packageStageObservationSequence,
    packageStageObservationsDropped,
    packageStageObservations: packageStageObservations.map((observation) => ({
      ...observation,
    })),
    ...packageEvents,
  };
...
export function resetPackageCachesForTests() {
  ...
  for (const localPackage of availableSourcePackages.values()) {
    if (sourcePackageOwnerCount(localPackage) !== 0) {
      throw new PackageCacheError(
  ...
  packageStageObservations.length = 0;
...
async function enforceLocalPackageCacheBounds() {
    ...
    const candidate = [...availableSourcePackages.entries()]
      .filter(
        ([, localPackage]) =>
          ...
          sourcePackageOwnerCount(localPackage) === 0,
      )
  ...
  // Wait for every selected deletion before allowing a retry to publish.
  const results = await Promise.allSettled(retirements);
  if (results.some((result) => result.status === "rejected")) {
    throw new PackageCacheError("Failed to clean retired packages");
  }
}
...
function retireSourcePackage(
  ...
  const retirement = observePackageCleanup("source", () =>
    fs.promises.rm(localPackage.dir, { recursive: true, force: true }),
  ).finally(() => {
...
function retireExternalPackage(
  ...
  const retirement = observePackageCleanup("external", () =>
    fs.promises.rm(localPackage.dir, { recursive: true, force: true }),
  ).finally(() => {
...
async function validateLocalSourcePackage(
  ...
  await observePackageValidation("source", phase, async () => {
...
async function validateLocalExternalPackage(
  local: { dir: string },
  phase: PackageValidationPhase,
): Promise<void> {
  await observePackageValidation("external", phase, async () => {
