...
When the pinned local Node runtime patch is also present, every Node analysis
attempt uses the serialized internal system child. System admission happens
before fresh package URLs are signed, and the child restores its baseline
environment after analysis. This isolates codegen work from live application
Node children; it does not add a codegen commit or deployment side effect.
