...
serves many `/invoke` requests during its lifetime. `execute` and `analyze`
acquire a package lease before importing user modules. The local resilience
patch also uses the same acquisition boundary through `/prepare` to materialize
a candidate's packages without importing an application module. Source
packages are materialized lazily under:
...
- Each source archive's `externalDepsStorageKey` must match the requested
  ...
  metadata fails instead of receiving the first caller's package. Current-entry
  identity is checked before reuse, and neither failure path removes a package
  that another invocation may still be using.
...
- Failed or superseded in-flight work cannot later publish over a successful
  retry.
- Source and external trees are checked for completeness before atomic
  publication. Prebuilt package trees receive the same check before cache
  registration. A current cache hit compares the immutable source and external
  keys and checksums, updates recency, and acquires ownership without traversing
  the filesystem. Warm lookup work is therefore independent of module count.
  Complete lease acquisition can still wait for unrelated zero-owner cache
  entries selected for bounded retirement.
  Unsupported deletion or mutation after publication requires a fresh executor
  generation; the cache does not perform recurring defensive scans.
...
- Downloads reject 90 MB or larger compressed archives before buffering them,
...
- Candidate preparation validates the complete request shape, acquires the same
  source-package lease, and carries no function path or arguments. Base `warm`
  preparation releases that lease after publication; the later pinned-pool
  runtime uses `resident` preparation to retain it for the generation. Both
  modes exercise source and external package publication without importing or
  invoking application code. A preparation failure is a typed endpoint result
  and cannot make a candidate ready.
...
- Cache enforcement removes ownership and map entries synchronously, starts
  independent source and external removals together, and waits for every
  selected removal before returning a failure. A retry therefore cannot publish
  while an earlier failed enforcement pass still has deletion work in flight.
...
- Dependency builds reject 230 MB or larger extracted trees before archive
  creation and stop archive output at 90 MB. These creation-side limits match
...
removal. A current registered directory is not deleted or replaced in response
to a cache hit. Bounded retirement can remove it only after both request and
resident ownership reach zero. External deletion or mutation of the tempdir is
unsupported and requires restarting the executor to obtain a fresh tempdir;
current hits do not scan for or repair that mutation.

## Package lifetime

The local lease covers the supported invocation, analysis, or preparation
boundary. The base `warm` preparation mode releases its lease after publication
because it does not evaluate application code. The package primitive supports a
separate resident-owner category without depending on pool routing or backend
package IDs. The later pinned-pool runtime adopts that category through the
explicit `resident` mode and retains one lease for the complete application
generation lifetime, while ordinary invocations retain their own leases. User
syscalls are disposed when an invocation or analysis boundary returns success,
an error, or a timeout. The Node timeout
race cannot cancel the underlying JavaScript promise, and timed-out work and
un-awaited callbacks are not supported after the boundary. A future lazy import
is safe while its invocation owns the lease. Work that resumes after the
boundary cannot rely on historical package files remaining present.
...
bytes, active and generation-resident source owners, registered stack roots,
package hit/publish/retire/failed-publication counters, and stack-format
invocation/frame/duration counters. It also carries a bounded sequence of exact
package acquisition, staged/prebuilt validation, and retirement-cleanup
observations. Rust de-duplicates the sequence per child generation and exports
`local_node_package_acquire_seconds`,
`local_node_package_validation_seconds`, and
`local_node_package_cleanup_seconds`. If more than 256 observations accumulate
between successful health snapshots, the overwritten count is exported in
`local_node_package_stage_observations_dropped_total`. A killed child can lose
observations that were never read, while Rust request and preparation timers
still report the outer failure. A failed-publication event covers a package
...
metadata. Ordinary invocation leases remain request scoped, while the pool
runtime extends one source and external ownership chain for each application
generation.
...
- Candidate preparation publishes the source and external packages without
  evaluating an application module, leaves no active package owner, and rejects
  extra request or package fields before downloading.
...
- Synthetic one-module and 32-module packages acquire and release production
  leases with zero completeness stats on current source-cache reuse. Current
  external-cache reuse likewise performs no stat beneath the published
  dependency path.
- Concurrent resident preparations retain one owner, join one initialization,
  reject a different package identity, reuse the retained package after archive
  authority becomes unavailable, and recover cleanly after failed initialization.
- A resident package and its transitively owned external dependency survive
  unrelated source and external turnover at the real cache-count limits, remain
  included in byte accounting, and become eligible for retirement after the
  resident lease releases.
- Acquisition timing records one terminal success or failure, retains material
  work across an ownership retry, copies health snapshots, and bounds the
  sequence buffer to 256 entries with explicit dropped-observation accounting.
...
- Current source hits do not inspect a removed bundled `_deps` chunk, source
  map, or `package.json`; unsupported mutation is reused until the executor
  generation is replaced. The generated package marker still matters because
  user modules are `.js` files that Node must import as ESM.
- If a newly staged source links to a cached external package whose
  `node_modules` was removed out of band, staged source validation fails without
  deleting that external package or an older source package that still links to
  it. The external-cache hit itself performs no completeness scan.
...
- Active source-package files survive turnover beyond the configured count,
  ...
  zero.
- Lease acquisition retries if a source cache hit retires before the
  asynchronous caller resumes. Source publication likewise retries if an
  external cache hit retires before its source owner resumes.
- Source publication reserves a cached external package before linking, and
  repeated failed source downloads keep successful external downloads within
  the external cache bounds.
...
```sh
npm exec --no -- vitest run src/source_package.test.ts src/build_deps.test.ts src/errors.test.ts --maxWorkers=1
```

The source-package suite includes deterministic filesystem-operation counts;
it does not use timing-sensitive warm-cache assertions.
