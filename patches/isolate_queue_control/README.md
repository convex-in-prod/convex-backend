...
- `ISOLATE_QUEUE_DELAY_SHED_THRESHOLD_MILLIS=300` with the default target
- `ISOLATE_QUEUE_HARD_MAX_AGE_MILLIS=5000`

When the adaptive-shed threshold is unset, it defaults to twice the configured
target. Setting only a non-default target therefore preserves the original
`2 * target` shedding behavior. An explicit threshold decouples overload
detection from adaptive shedding.
