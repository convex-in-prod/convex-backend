...
child, retained generation-scoped package ownership, and exact resident-generation admission.
Failure before `ready` drops the preparation while
...
`/invoke`. The successful preparation acknowledgment proves that the selected
application generation retains the complete source and external package
identity through the claim. A future remote executor would need its own
admission protocol.
...
Resident `/prepare` retains one source-package lease in the selected application generation before
returning success. That lease owns the linked external package and survives the durable claim, so
bounded cache enforcement and signed-URL expiry do not reopen package acquisition before
`/invoke`. Ordinary invocation leases remain request scoped. The complete package-lifetime
