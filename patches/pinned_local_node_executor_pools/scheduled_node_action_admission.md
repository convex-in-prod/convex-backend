...
linear active-request guard and no new durable state. Resident preparation now
retains the selected generation's package lease across the durable claim and
through every invocation in that generation.
...
Package preparation acknowledges `ready` only after the child has retained one
generation-scoped source lease for the complete source and external identity.
A legacy preparation success without the residency acknowledgment fails closed.
The generation fingerprint selects the intended identity but does not itself
prove that this ownership was established; a cold generation published before
its first preparation remains unready until the validated retained response.
...
### Package authority is retained by the generation

`/prepare` has separate `warm` and `resident` modes. Application generations
use `resident`. The child installs an initializing identity before awaiting any
package work, joins concurrent preparation for that identity, and stores one
lease after publication and validation succeed. A different source or external
identity fails explicitly in the same generation. Failed initialization waits
for cleanup, returns to the uninitialized state, and can be retried.

The resident source lease owns the linked external package transitively and is
released only when the child generation exits. Ordinary invocations still take
their own request leases. Once resident readiness is confirmed, a delayed claim
and later `/invoke` do not depend on the signed archive URL remaining valid.
Cache pressure cannot retire the resident package, and repeated readiness does
not add another resident owner.
...
controller cancellation, package preparation failure, and known generation
loss. Existing router and durable-admission metrics continue to
...
Resident preparation also retains one child package-cache lease for the
generation lifetime.
...
exits use bounded outcomes for cancellation, package preparation failure, and
generation loss.
...
unavailability canceling it, preparation failure before `ready`, retained
resident reuse after signed authority expires, and no-op rejection before
`ready`. Child tests cover concurrent resident
initialization, one retained owner, identity mismatch, failed initialization
and retry, unavailable archive authority after readiness, and rejection of a
legacy success response without residency proof.
...
the action remains pending for a fresh retry. The admitted generation also owns
the exact package files across the claim, so ordinary cache retirement and URL
expiry do not reopen package acquisition after `ready`.
