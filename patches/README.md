...
- Purpose: retire a selected local Node generation on request/stream timeout, transport failure,
  ...
  mechanism with retire-first cgroup-pressure handling. The composed pinned runtime uses the same
  lifecycle owner for a dedicated serialized system child and retains system-operation capacity
  through child completion when a caller disappears. This patch also moves the local runtime to
...
- Purpose: let a root Node action module require a bounded named one-process local executor pool.
  ...
  replace a newer resident generation. Default and named application generations retain one exact
  source/external package owner for their child lifetime. Other actions continue through the
  default executor. Analysis and dependency builds use a separate `_system` child with one active
  operation, eight FIFO waiters, and a 60-second admission deadline; signing occurs after admission.
...
- Activation: first set `LOCAL_NODE_EXECUTOR_TOTAL_RSS_BUDGET_BYTES` to cover the default
  ...
  candidate or a superseded draining old generation after an explicit interruption warning. The
  optional per-pool admission and RSS policy is documented in
  [`local_node_executor_pool_admission`](local_node_executor_pool_admission/README.md) and is
  part of this runtime-pools adoption unit.
